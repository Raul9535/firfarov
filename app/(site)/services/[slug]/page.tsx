...
/**
 * EN service detail. Looks the document up by `slugEn.current == $slug` via
 * `serviceBySlugQuery` (which takes a `$locale` param and matches the right slug field).
 *
 * If no service has the given EN slug — `notFound()` so Next renders the proper 404 page
 * instead of an empty shell. Otherwise renders the title + tagline at hero scale; the
 * positioning paragraph follows in the home's 4|8 spine if filled.
 *
 * Other approved sections (who-it's-for, process, deliverables, …) come later as the
 * service-detail layout is built out — for MVP this gets the seeded services to actually
 * resolve at /services/[slug].
 */
